...
- `istio/` — Istio ambient mesh: the identity/mTLS layer (SPIFFE per-agent identity + A2A authz).
  `istio/policies/` holds the AuthorizationPolicy allow-lists (identity-based access control).
- `external-secrets/` — External Secrets Operator: syncs secrets from AWS Secrets Manager into k8s via
  git-committed ExternalSecret pointers (Pod Identity auth, no static keys). Backing store in `infra/03-argocd/eso.tf`.
